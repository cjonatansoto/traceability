...
class DispatchGuideController extends ApiController
{
    public function getById(Request $request)
    {
        ...
        $sumKgBins = StockLot::where('dispatch_guide_id', $dispatchguide->id)->where('quantity_type_id', 2)->sum('kg_amount');

        $sumKgBoxes = StockLot::where('dispatch_guide_id', $dispatchguide->id)->where('quantity_type_id', 1)->sum('kg_amount');

        $sumKgTotal = StockLot::where('dispatch_guide_id', $dispatchguide->id)->sum('kg_amount');
