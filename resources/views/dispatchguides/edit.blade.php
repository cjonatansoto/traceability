                        ...
                        <div class="card-body">
                            <form id="sendEditDispatchGuide">
                                <div class="col-xs-12 col-sm-12 col-md-12">
                                    ...
                                    <div class="row">
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" id="dispatch_guide_type_id">
                                                    ...
                                                    @foreach($dispatchguidetypes as $item)
                                                        @if($item->id == $dispatchguide->dispatch_guide_type_id)
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" id="number" value="{{$dispatchguide->number}}" onkeypress="return valideKey(event);" disabled>
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <input type="datetime-local" class="form-control" id="plant_entry_date" max="{{ date('Y-m-d\TH:i') }}" value="{{date('Y-m-d\TH:i', strtotime($dispatchguide->plant_entry_date))}}" >
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <input type="date" class="form-control" id="date_physical_guide" max="{{ now()->toDateString('Y-m-d') }}" value="{{date('Y-m-d', strtotime($dispatchguide->date_physical_guide))}}" >
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <input type="date" class="form-control" id="target_date" max="{{date('Y-m-d', strtotime(' + 30 days'))}}" value="{{date('Y-m-d', strtotime($dispatchguide->target_date))}}">
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                <label>Empresa</label>
                                                <select class="form-control selected2" id="enterprise_id">
                                                    <option value="" selected disabled hidden>SELECCIONAR...</option>
                                                    @foreach($enterprises as $enterprise)
                                                        @if($dispatchguide->enterprise_id == $enterprise->cod_empresa)
                                        ...
                                        <div class="col-md-4">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control selected2" id="provider_id">
                                                    <option value="" selected disabled hidden>SELECCIONAR...</option>
                                                    @foreach($providers as $provider)
                                                        @if($dispatchguide->provider_id == $provider->cod_proveedor)
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <table class="table table-striped table-bordered" style="width:100%" id="items">
                                                ...
                                                @foreach($dispatchguideitems as $items)
                                                    <tr quantity_type_id={{$items->quantityType->id}} amount={{$items->amount}} pieces={{$items->pieces}}
                                                        kgs={{$items->kgs}}
                                                        species_id={{$items->species->cod_especie}} cut_id={{$items->court->cod_corte}}
                                                        preservation_id={{$items->preservation->cod_cons}}>
                                                        <td> {{$items->quantityType->name}}</td>
                                                        <td> {{$items->amount}}</td>
                                                        <td> {{$items->pieces}}</td>
                                                        <td> {{number_format($items->kgs, 3, '.', '')}}</td>
                                                        <td> {{$items->species->descripcion}}</td>
                                                        <td> {{$items->court->nombre}}</td>
                                                        <td> {{$items->preservation->nombre}}</td>
                                                        <td class="align-content-center justify-content-center text-center">
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control" rows="5" id="observations" style="height: 100px">{{$dispatchguide->observations}}</textarea>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <button class="btn btn-primary" type="submit" id="btnUpdate">
                                                Guardar
                                            </button>
                                        </div>
                                    </div>
...
@push('scripts')
    <script type="text/javascript">
        ...
        $('#sendEditDispatchGuide').on('submit',function(e) {
            ...
            $.ajax({
                url: "{{route('dispatchguides.update', $dispatchguide->id)}}",
                type: "POST",
                cache: false,
                contentType: false,
                processData: false,
